...
driver.get('file:///D:/Users/Administrator/PycharmProjects/ixibanyayu.com/a2.html')
a2csv = pandas.DataFrame(columns=['index_spainish', 'chinese only', 'spainish'])
index = 406
...
import os
if os.path.exists("a2.mp3"):
  os.remove("a2.mp3")
# copy('a1.mp3','a2.mp3')
with open('a2.mp3', 'wb') as f:
    ...
    for tr in driver.find_element_by_css_selector("#post-8089 > div.post-content.container-fluid.d-flex.justify-content-center.mb-5 > div > div > table").find_elements_by_tag_name("tr"):
        ...
        if len(tds) == 3:
            ...
            sleep(2)
    a2csv.to_excel("a2.xls", index=False)
pass