...
driver.get('file:///D:/Users/Administrator/PycharmProjects/ixibanyayu.com/sentences100.html')
sentences100csv = pandas.DataFrame(columns=['index_spainish', 'chinese', 'spainish'])
index = 0
...
import os
if os.path.exists("sentences100.mp3"):
  os.remove("sentences100.mp3")
with open('sentences100.mp3', 'wb') as f:
    # for i in range(0, 100):
    #     gTTS(str(i), lang="zh-TW").write_to_fp(f)
    #     gTTS(str(i), lang="es", tld='com').write_to_fp(f)

    for tr in driver.find_element_by_css_selector("#post-10431 > div.post-content.container-fluid.d-flex.justify-content-center.mb-5 > div > div > table").find_elements_by_tag_name("tr"):
        ...
        if len(tds) == 2:
            ...
            print(index_spainish, chinese, spainish)
            sentences100csv = sentences100csv.append({
            ...
            sleep(2)
    sentences100csv.to_excel("sentences100.xls", index=False, header=False)
pass